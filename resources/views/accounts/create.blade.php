...
<head>
    ...
    <title>Buat Akun - Patria Maritim Perkasa</title>
...
<body class="bg-gray-50 text-gray-900 min-h-screen flex">
    ...
    <aside class="bg-white w-64 min-h-screen border-r border-gray-200 flex flex-col">
        ...
        <nav class="flex-1 p-4">
            <div class="space-y-2">
                ...
                @if (Auth::user()->hasAnyRole(['admin', 'team_hc']))
    ...
    <main class="flex-1 p-6">
        <div class="max-w-2xl mx-auto">
            <h1 class="text-2xl font-semibold text-gray-900 mb-6">Buat Akun Baru</h1>
            ...
            <form action="{{ route('accounts.store') }}" method="POST" class="bg-white shadow-md rounded-lg p-6">
                @csrf
                <div class="grid gap-6">
                    <div>
                        <label for="name" class="block text-sm font-medium text-gray-700">Nama Lengkap</label>
                        <input type="text" name="name" id="name" value="{{ old('name') }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                    </div>
                    <div>
                        <label for="email" class="block text-sm font-medium text-gray-700">Email</label>
                        <input type="email" name="email" id="email" value="{{ old('email') }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <select name="role" id="role" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                            ...
                            <option value="team_hc">Team HC</option>
                        </select>
                    </div>
                    <div>
