...
<div class="bg-white rounded-xl p-4 sm:p-6 border border-gray-200 shadow-sm">
    <form action="{{ route('vacancies.update', $vacancy->id) }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <input type="number" name="needed_count" id="needed_count" min="0" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" value="{{ old('needed_count', $vacancy->needed_count) }}" required>
            @error('needed_count')
                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
